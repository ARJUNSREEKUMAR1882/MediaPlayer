...
import { addCategoryAPI,getVideoAPI, getCategoryAPI, removeCategoryAPI, updateCategoryAPI } from '../services/allAPI';
...
function Category({removeCategoryVideoResponse}) {
  ...
  const handleAddCategory = async () => {
    if (categoryName) {
      const result = await addCategoryAPI({ categoryName, Videos: [] })
      handleClose()
      getAllcategories()
    } else {
        ...
        {allCategories.length>0? allCategories.map((item, index) => (
          <div droppable="true" onDragOver={(e) => dragOverCategory(e)} onDrop={(e) => videoDropped(e, item?.id)} key={index} className='border rounded p-3 mb-2'>
            ...
            <div className="row mt-2">
              {
                item.Videos?.length>0 && item.Videos?.map((video, index) => 
                (
