  ...
  return (
    <div className='container mt-5 mb-5'>
      ...
      <table className='table mt-5 mb-5'>
        ...
        <tbody>
          {history?.length > 0 ? history?.map((videos, index) => (
            <tr key={index}>
              <td>{index + 1}</td>
              <td>{videos.caption}</td>
              <td><a href={videos?.youtubeLink} target='_blank'>{videos?.youtubeLink}</a></td>
              <td>{videos?.timeStamp}</td>
              <td><button onClick={()=>deleteHistory(videos.id)} className='btn'><i className='fa-solid fa-trash text-danger'></i></button></td>
            </tr>
          ))
